...
static uint64_t linux_msr_read(uint64_t index)
{
	...
	if (msr_fd == -1) {
		os_debug(
			"Could not open MSR file; are you root? did you modprobe msr?");
		abort();
	}
...
void os_init(void)
{
	...
	if (ioperm(0x80, 1, 1) < 0 || ioperm(PCI_CONFIG_ADDR, 4, 1) < 0 ||
	    ioperm(PCI_CONFIG_DATA, 4, 1) < 0) {
	...
	static_assert(
		HUGEPAGE_SIZE >= OS_MEMORY_SIZE,
		"HUGEPAGE_SIZE should not be smaller than OS_MEMORY_SIZE");
	memory = mmap(
		// No specific address
		NULL,
		// Size of the mapping
		...
		MAP_HUGETLB | (HUGEPAGE_SIZE_POWER << MAP_HUGE_SHIFT) |
